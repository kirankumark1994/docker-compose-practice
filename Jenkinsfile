            ...
            steps{
                echo "======== Executing Deploy========"
                    //withCredentials([sshUserPrivateKey(credentialsId: 'docker-machine-ssh-key', keyFileVariable: 'docker-ubuntu', usernameVariable: 'ubuntu')]) {
                      sh(script:"ssh -o StrictHostKeyChecking=no  ubuntu@18.205.158.137 \"mkdir -p /home/ubuntu/compose-deployment/ \" ")
                      sh(script:"scp -r ${WORKSPACE}/* ubuntu@18.205.158.137:/home/ubuntu/compose-deployment/ ")
                      sh(script:"ssh -o StrictHostKeyChecking=no  ubuntu@18.205.158.137 \"cd /home/ubuntu/compose-deployment/ && sudo docker-compose up -d \" ")
                //}
